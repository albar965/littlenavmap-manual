...
Note that you have to restart *Little Navmap* to see any changes. Also,
...
``Version`` as shown below. ``INI [Options] Version=2.0.1`` **Do not
delete this since Little Navmap relies on this information when updating
...
**Add this version section to all new files. Otherwise,**\ \_Little
Navmap_*\* might reset the content.*\*
...
and ``Night`` and can be found in the dialog `User
Interface <OPTIONS.html#user-interface>`__ tab of the options dialog.
